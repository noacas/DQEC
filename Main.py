...
class QECC_Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        syndrome = (torch.matmul(torch.from_numpy(noise_cumulative).long().transpose(0, 1),
                                 self.pc_matrix) % 2).transpose(0, 1).numpy()
...
def main(args):
    ...
    train_dataloader = DataLoader(QECC_Dataset(code, ps_train, len=args.batch_size * 5000, args=args),
